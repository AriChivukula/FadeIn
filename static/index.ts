...
window.onload = (): void => {
  ...
  DB.vertices.filter((vert: any) => vert.label === "VL-scene").forEach(
    (vert: any) => {
      const lMatches = DB.vertices.filter((vert: any) => vert.label === "VL-location").filter(vert => vert.id === vert.properties.location);
      ...
      vert.properties.lines.forEach(
        (line: string) => {
          const cMatches = DB.vertices.filter((vert: any) => vert.label === "VL-character").filter(vert => vert.id === line);
